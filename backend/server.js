...
// Яндекс возвращает браузер обратно на /api/auth/yandex/callback.
...
// устройство, которое ждёт результат через опрос (см. /api/oauth/yandex-status),
...
function buildYandexAuthUrl(state) {
...
// Обмен кода на access_token, затем сразу — на данные пользователя (uid).
// Две последовательных сетевых операции, обе к серверам Яндекса.
async function yandexExchangeCodeForUid(code) {
  ...
  if (!tokenRes.ok) throw new Error('Яндекс не подтвердил код авторизации');
  const tokenData = await tokenRes.json();
  const infoRes = await fetch('https://login.yandex.ru/info?format=json', {
    headers: { Authorization: `OAuth ${tokenData.access_token}` },
  });
  if (!infoRes.ok) throw new Error('Не удалось получить данные аккаунта Яндекса');
  const info = await infoRes.json();
  return String(info.id); // стабильный числовой uid — именно он используется как id в login.yandex.ru/info
}
...
async function initDatabase() {
  ...
  await db.exec(`ALTER TABLE users ADD COLUMN IF NOT EXISTS yandex_uid TEXT`);
...
// Ключи приложения — через переменные окружения (задаются в самом
// systemd-юните, см. документацию), а не в исходном коде.
const YANDEX_CLIENT_ID = process.env.YANDEX_CLIENT_ID;
const YANDEX_CLIENT_SECRET = process.env.YANDEX_CLIENT_SECRET;
// Публичный адрес приложения — единая точка настройки при переносе на другой
// домен. От него зависят: разрешённый источник CORS, адрес обратного вызова
// Яндекс OAuth и адрес возврата после подтверждения.
const APP_URL = (process.env.APP_URL || 'http://localhost:3000').replace(/\/+$/, '');
const YANDEX_REDIRECT_URI = process.env.YANDEX_REDIRECT_URI || (APP_URL + '/api/auth/yandex/callback');
if (!YANDEX_CLIENT_ID || !YANDEX_CLIENT_SECRET) {
  console.warn('⚠ YANDEX_CLIENT_ID/YANDEX_CLIENT_SECRET не заданы — вход через Яндекс ID как второй фактор работать не будет, пока их не задать.');
}
...
app.use((req, res, next) => {
  ...
  res.setHeader('Content-Security-Policy', [
    ...
    "connect-src 'self' https://oauth.yandex.ru https://login.yandex.ru",
...
// входом (когда Яндекс ID не привязан), и колбэком /api/auth/yandex/callback
...
async function issueSessionToken(row) {
  ...
  return Object.assign({ token: token, scopeDivisions: divRows.map(r => r.div_name), avatar: row.avatar || null, yandexLinked: !!row.yandex_uid }, payload);
}
...
app.post('/api/login', checkLoginRateLimit, async (req, res) => {
  ...
  if (row.yandex_uid) {
    // Пароль верный, но это ещё не полноценный вход — второй фактор
    // подтверждается через Яндекс ID. State целиком хранится на сервере
    // и уже содержит username, редирект просто уводит браузер на Яндекс
    // и обратно на /api/auth/yandex/callback.
    await recordLoginSuccess(req.ip, username);
    const state = await createOauthState('verify', row.username);
    return res.json({ requiresYandex: true, yandexAuthUrl: buildYandexAuthUrl(state), state: state });
  }
  await recordLoginSuccess(req.ip, username);
...
app.get('/api/me', authMiddleware, async (req, res) => {
  ...
  const row = await db.get('SELECT avatar, yandex_uid FROM users WHERE username = ?', req.user.username);
  res.json(Object.assign({}, req.user, { scopeDivisions: divRows.map(r => r.div_name), avatar: (row && row.avatar) || null, yandexLinked: !!(row && row.yandex_uid) }));
});
...
// ── Яндекс ID — второй фактор при входе ─────────────────────────────────
app.post('/api/yandex/link/start', authMiddleware, async (req, res) => {
  if (!YANDEX_CLIENT_ID || !YANDEX_CLIENT_SECRET) {
    return res.status(503).json({ error: 'Вход через Яндекс ID не настроен на сервере' });
  }
  const state = await createOauthState('link', req.user.username);
  res.json({ authUrl: buildYandexAuthUrl(state), state: state });
});

app.post('/api/yandex/unlink', authMiddleware, async (req, res) => {
  const { password } = req.body || {};
  if (!password) return res.status(400).json({ error: 'Укажите пароль для подтверждения' });
  const row = await db.get('SELECT password_hash FROM users WHERE username = ?', req.user.username);
  if (!row || !bcrypt.compareSync(password, row.password_hash)) {
    return res.status(401).json({ error: 'Пароль указан неверно' });
  }
  await db.run('UPDATE users SET yandex_uid = NULL WHERE username = ?', req.user.username);
  res.json({ unlinked: true });
});

// Единая точка возврата из Яндекса и для привязки, и для входа — какой из
// двух сценариев это был, определяем по purpose внутри state, а не по URL.
app.get('/api/auth/yandex/callback', async (req, res) => {
  ...
  if (error || !code || !state) {
    ...
    return res.redirect(redirectBase + '?yandex=error');
  }
  const entry = await lockOauthStateForProcessing(state);
  if (!entry) {
    return res.redirect(redirectBase + '?yandex=expired');
  }
  let yandexUid;
  try {
    yandexUid = await yandexExchangeCodeForUid(code);
  } catch (e) {
  ...
  if (entry.purpose === 'link') {
    // Один и тот же Яндекс-аккаунт нельзя привязать к двум разным локальным
    // пользователям одновременно — иначе один вход открывал бы доступ сразу
    // к нескольким учёткам LEMANAPRO.
    const already = await db.get('SELECT username FROM users WHERE yandex_uid = ? AND username != ?', yandexUid, entry.username);
    if (already) {
      await completeOauthState(state, 'error', { code: 'taken' });
      return res.redirect(redirectBase + '?yandex=taken');
    }
    await db.run('UPDATE users SET yandex_uid = ? WHERE username = ?', yandexUid, entry.username);
    await completeOauthState(state, 'done', { linked: true });
    return res.redirect(redirectBase + '?yandex=linked');
  }

  // purpose === 'verify' — второй шаг входа
  const row = await db.get('SELECT * FROM users WHERE username = ?', entry.username);
  if (!row || row.yandex_uid !== yandexUid) {
    await recordLoginFailure(req.ip, entry.username);
    await completeOauthState(state, 'error', { code: 'mismatch' });
    return res.redirect(redirectBase + '?yandex=mismatch');
  }
  await recordLoginSuccess(req.ip, entry.username);
  const sessionData = await issueSessionToken(row);
  await completeOauthState(state, 'done', sessionData);
  // Токен передаём через query-параметр разового обмена — фронтенд, увидев
  // yandexToken при загрузке, сразу меняет его на localStorage и чистит URL
  // через history.replaceState, чтобы он не оставался в истории браузера.
  // Актуально для сценария «подтвердили на этом же устройстве» (кнопка);
  // при сканировании QR другим устройством ждущий рабочий стол получит тот
  // же sessionData через опрос ниже, а этот редирект просто откроет вход
  // ещё и на устройстве, которым сканировали, что не мешает.
  return res.redirect(redirectBase + '?yandexToken=' + encodeURIComponent(sessionData.token));
});

// Опрос статуса — для варианта «отсканировали QR другим устройством»:
// рабочий стол, показавший QR, спрашивает раз в несколько секунд, не
// подтвердили ли уже на телефоне. Сам state — единственное, что нужно для
// этого запроса (это разовый неугадываемый токен, а не пароль или сессия).
app.get('/api/oauth/yandex-status', async (req, res) => {
...
app.get('/api/users', authMiddleware, requireUnrestrictedDeveloper, async (req, res) => {
  const rows = await db.all('SELECT username, display_name, role, created_at, yandex_uid FROM users ORDER BY created_at');
  ...
  res.json(rows.map(u => Object.assign({}, u, { scopeDivisions: scopeByUser[u.username] || [], yandexLinked: !!u.yandex_uid, yandex_uid: undefined })));
});
...
// Сброс привязки Яндекс ID у ДРУГОГО пользователя — на случай, если человек
// потерял доступ и к паролю от своего аккаунта Яндекса одновременно, и сам
// снять привязку (что требует ввода пароля LEMANAPRO) не может. После сброса
// пользователь сможет войти по одному паролю и привязать Яндекс ID заново.
app.post('/api/users/:username/reset-yandex', authMiddleware, requireUnrestrictedDeveloper, async (req, res) => {
  const { username } = req.params;
  const target = await db.get('SELECT username, yandex_uid FROM users WHERE username = ?', username);
  if (!target) return res.status(404).json({ error: 'Пользователь не найден' });
  if (!target.yandex_uid) return res.status(400).json({ error: 'У этого пользователя Яндекс ID не привязан' });
  await db.run('UPDATE users SET yandex_uid = NULL WHERE username = ?', username);
  res.json({ username, yandexReset: true });
});
