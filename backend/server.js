...
// ── Обмен с NetBox ───────────────────────────────────────────────────────
// Два токена намеренно раздельные: чтение нужно для сверки и работает
// постоянно, запись — только при отправке. Разделение позволяет отозвать
// право записи, не останавливая сверку.
const NETBOX_URL = (process.env.NETBOX_URL || '').replace(/\/+$/, '');
const NETBOX_TOKEN_READ = process.env.NETBOX_TOKEN_READ;
const NETBOX_TOKEN_WRITE = process.env.NETBOX_TOKEN_WRITE;
...
const netboxReadEnabled = Boolean(NETBOX_URL && NETBOX_TOKEN_READ);
const netboxWriteEnabled = Boolean(NETBOX_URL && NETBOX_TOKEN_WRITE);

if (!netboxReadEnabled) {
  console.warn('⚠ Обмен с NetBox не настроен — сверка и отправка недоступны.');
} else if (!netboxWriteEnabled) {
  console.warn('⚠ Токен записи NetBox не задан — доступна только сверка.');
}
 ...
 * @param {object} options — метод и тело
 * @param {boolean} write — использовать токен записи
 */
async function netboxFetch(path, options = {}, write = false) {
  const token = write ? NETBOX_TOKEN_WRITE : NETBOX_TOKEN_READ;
  if (!token) throw new Error(write ? 'Токен записи NetBox не задан' : 'Токен чтения NetBox не задан');
...
async function netboxFetchDevices(query) {
  const devices = [];
  let path = `/api/dcim/devices/?limit=200&${query}`;
...
app.get('/api/audit', authMiddleware, requireUnrestrictedDeveloper, async (req, res) => {
  ...
  const rows = await db.all(
    'SELECT ts, username, action, target, details, ip FROM audit_log ORDER BY ts DESC LIMIT ?',
    limit
  );
  res.json(rows);
});
...
// Предварительный просмотр: что будет отправлено. Ничего не меняет.
app.get('/api/netbox/preview', authMiddleware, requireEdit, async (req, res) => {
  if (!netboxReadEnabled) {
    return res.status(503).json({ error: 'Обмен с NetBox не настроен на сервере' });
  }

  const mag = String(req.query.mag || '').trim();
  if (!mag) return res.status(400).json({ error: 'Укажите объект' });

  const scopeErr = await checkMagScope(req.user.username, mag);
  if (scopeErr) return res.status(403).json({ error: scopeErr });

  try {
    const plan = await buildNetboxPlan(mag);
    res.json(plan);
  } catch (e) {
    console.error('Не удалось построить план отправки:', e.message);
    res.status(502).json({ error: 'Не удалось получить данные из NetBox: ' + e.message });
  }
});

// Отправка. Выполняется по одному объекту.
app.post('/api/netbox/export', authMiddleware, requireEdit, async (req, res) => {
  if (!netboxWriteEnabled) {
    return res.status(503).json({ error: 'Токен записи NetBox не задан на сервере' });
  }

  const mag = String((req.body && req.body.mag) || '').trim();
  if (!mag) return res.status(400).json({ error: 'Укажите объект' });
  ...
  let plan;
  try {
    plan = await buildNetboxPlan(mag);
  } catch (e) {
    return res.status(502).json({ error: 'Не удалось получить данные из NetBox: ' + e.message });
  }

  if (!plan.toSend.length) {
    return res.json({ mag, sent: 0, failed: 0, message: 'Отправлять нечего — расхождений нет.' });
  }
  ...
  for (const item of plan.toSend) {
    try {
      // Изменяется ТОЛЬКО поле серийного номера: остальные сведения
      // об устройстве ведёт другое подразделение и трогать их нельзя.
      await netboxFetch(
        `/api/dcim/devices/${item.deviceId}/`,
        { method: 'PATCH', body: JSON.stringify({ serial: item.newSerial }) },
        true
      );
      sent.push(item);
    } catch (e) {
      failed.push({ sw: item.sw, error: e.message });
      console.error(`Не удалось обновить ${item.sw} в NetBox:`, e.message);
    }
    await sleep(NETBOX_REQUEST_DELAY_MS);
  }

  await writeAudit(req, 'netbox_export', mag, {
    отправлено: sent.length,
    перезаписано: sent.filter((x) => x.overwrites).length,
    ошибок: failed.length,
    записи: sent.map((x) => ({ sw: x.sw, было: x.oldSerial, стало: x.newSerial })),
  });

  res.json({
    mag,
    sent: sent.length,
    overwritten: sent.filter((x) => x.overwrites).length,
...
// ── Отправка стоек и размещения в NetBox ─────────────────────────────────
// Отправляются сами стойки и привязка коммутаторов к ним. Пассивное
// оборудование (патч-панели, органайзеры) не отправляется: в нашем учёте
// это строки текста, а в NetBox стойка содержит устройства с моделью
// и производителем — их пришлось бы заводить отдельно.

/**
 * Нижний юнит устройства.
 *
 * У нас многоюнитовое устройство записано несколькими строками подряд
 * с одинаковым наименованием: занимающее два юнита лежит и на 10-м,
 * и на 9-м. В NetBox позиция — это НИЖНИЙ юнит, поэтому из группы
 * берётся наименьший номер. Отправив верхний, мы разместили бы
 * устройство на юнит-два выше, чем оно стоит.
 */
function bottomUnitOf(items, item) {
  let unit = Number(item.unit);
  while (true) {
    const below = items.find(
      (x) => Number(x.unit) === unit - 1 &&
             x.name === item.name &&
...
function rackHeightOf(characteristic) {
  const match = String(characteristic || '').match(/(\d+)\s*u/i);
  return match ? Number(match[1]) : null;
}

/**
 * План отправки стоек и размещения по объекту.
 *
 * Площадка для создания стойки берётся из устройств этого же объекта
 * в NetBox: собственных сведений о площадках у нас нет, а угадывать
 * нельзя — стойка попала бы не туда.
 */
async function buildRackPlan(mag) {
  const cabinets = await db.all('SELECT * FROM cabinets WHERE mag = ?', mag);
  if (!cabinets.length) {
    return { mag, racks: [], placements: [], moves: [], skipped: [], summary: { стоек: 0 } };
  }
  ...
  for (const device of devices) deviceByName.set(normalizeDeviceName(device.name), device);

  // Площадка определяется по устройствам объекта. Если они стоят
  // на разных площадках, это расхождение в данных — отправку по такому
  // объекту лучше не делать вслепую.
  const sites = new Set();
  const magSwitches = await db.all('SELECT sw FROM switches WHERE mag = ?', mag);
  for (const row of magSwitches) {
    const device = deviceByName.get(normalizeDeviceName(row.sw));
    if (device && device.site) sites.add(device.site.id);
  }

  const siteId = sites.size === 1 ? [...sites][0] : null;
  const siteProblem = sites.size === 0
    ? 'Не удалось определить площадку: ни один коммутатор объекта не найден в NetBox'
    : sites.size > 1
      ? `Коммутаторы объекта числятся на разных площадках (${sites.size}) — отправка стоек невозможна`
      : null;

  // Существующие стойки этой площадки
  const existingRacks = siteId
    ? (await netboxFetch(`/api/dcim/racks/?site_id=${siteId}&limit=500`)).results || []
    : [];
  const rackByName = new Map();
  for (const rack of existingRacks) rackByName.set(normalizeDeviceName(rack.name), rack);
  ...
  for (const cabinet of cabinets) {
    const existing = rackByName.get(normalizeDeviceName(cabinet.name));
    const height = rackHeightOf(cabinet.characteristic);
    ...
    racks.push({
      ...
      action: existing ? 'существует' : 'будет создана',
    });

    const items = await db.all('SELECT unit, name, comment FROM cabinet_items WHERE cabinet_id = ?', cabinet.id);
    const annotated = await annotateCabinetItems(mag, cabinet.name, items);

    // Одно устройство занимает несколько строк — отправляем один раз
    const seen = new Set();

    for (const item of annotated) {
      if (!item.matchedSw) {
        skipped.push({ шкаф: cabinet.name, юнит: item.unit, позиция: item.name, причина: 'не коммутатор' });
        continue;
      }
      if (seen.has(item.matchedSw)) continue;

      const device = deviceByName.get(normalizeDeviceName(item.matchedSw));
      if (!device) {
        skipped.push({ шкаф: cabinet.name, юнит: item.unit, позиция: item.name, причина: 'нет в NetBox' });
        continue;
      }

      seen.add(item.matchedSw);
      const position = bottomUnitOf(items, item);

      const currentRack = device.rack ? device.rack.name : null;
      const currentPosition = device.position != null ? Number(device.position) : null;

      const sameRack = currentRack && normalizeDeviceName(currentRack) === normalizeDeviceName(cabinet.name);
      const samePosition = currentPosition === position;

      if (sameRack && samePosition) continue; // уже так и записано

      const entry = {
        sw: item.matchedSw,
        deviceId: device.id,
        rack: cabinet.name,
        rackId: existing ? existing.id : null,
        position,
        currentRack,
        currentPosition,
      };

      // Перемещение между стойками выделяется отдельно: это
      // существенное изменение, и делать его заодно с остальным
      // без ведома человека неправильно.
      if (currentRack && !sameRack) moves.push(entry);
      else placements.push(entry);
    }
  }
  ...
  return {
    ...
    summary: {
      стоек: racks.length,
      будетСоздано: racks.filter((r) => !r.existingId).length,
...
// Предварительный просмотр отправки стоек.
app.get('/api/netbox/racks/preview', authMiddleware, requireEdit, async (req, res) => {
  if (!netboxReadEnabled) {
    return res.status(503).json({ error: 'Обмен с NetBox не настроен на сервере' });
  }
  const mag = String(req.query.mag || '').trim();
  ...
  try {
    res.json(await buildRackPlan(mag));
  } catch (e) {
    console.error('Не удалось построить план по стойкам:', e.message);
    res.status(502).json({ error: 'Не удалось получить данные из NetBox: ' + e.message });
  }
});

// Отправка стоек и размещения.
app.post('/api/netbox/racks/export', authMiddleware, requireEdit, async (req, res) => {
  if (!netboxWriteEnabled) {
    return res.status(503).json({ error: 'Токен записи NetBox не задан на сервере' });
  }

  const mag = String((req.body && req.body.mag) || '').trim();
  // Перемещения между стойками отправляются только по отдельному
  // указанию: молча переносить устройство в другую стойку нельзя.
  const includeMoves = Boolean(req.body && req.body.includeMoves);
  ...
  let plan;
  try {
    plan = await buildRackPlan(mag);
  } catch (e) {
    return res.status(502).json({ error: 'Не удалось получить данные из NetBox: ' + e.message });
  }

  if (plan.siteProblem) {
    return res.status(400).json({ error: plan.siteProblem });
  }
  ...
  // 1. Стойки, которых ещё нет
  for (const rack of plan.racks) {
    if (rack.existingId) continue;
    try {
      const body = { name: rack.name, site: plan.siteId, status: 'active' };
      if (rack.height) body.u_height = rack.height;
      const result = await netboxFetch('/api/dcim/racks/', { method: 'POST', body: JSON.stringify(body) }, true);
      rack.existingId = result.id;
      created.push(rack.name);
    } catch (e) {
      failed.push({ объект: rack.name, ошибка: e.message });
    }
    await sleep(NETBOX_REQUEST_DELAY_MS);
  }

  // Идентификаторы созданных стоек нужны для размещения
  const rackIdByName = new Map(plan.racks.map((r) => [normalizeDeviceName(r.name), r.existingId]));

  // 2. Размещение устройств
  const toPlace = includeMoves ? [...plan.placements, ...plan.moves] : plan.placements;

  for (const item of toPlace) {
    const rackId = item.rackId || rackIdByName.get(normalizeDeviceName(item.rack));
    if (!rackId) {
      failed.push({ объект: item.sw, ошибка: 'стойка не создана' });
      continue;
    }
    try {
      // face обязателен при указании позиции — иначе NetBox отклонит
      await netboxFetch(
        `/api/dcim/devices/${item.deviceId}/`,
        { method: 'PATCH', body: JSON.stringify({ rack: rackId, position: item.position, face: 'front' }) },
        true
      );
      placed.push(item);
    } catch (e) {
      failed.push({ объект: item.sw, ошибка: e.message });
    }
    await sleep(NETBOX_REQUEST_DELAY_MS);
  }

  await writeAudit(req, 'netbox_racks_export', mag, {
    созданоСтоек: created.length,
    размещено: placed.length,
    перемещенийВключено: includeMoves,
    ошибок: failed.length,
    записи: placed.map((x) => ({ sw: x.sw, стойка: x.rack, юнит: x.position, былаСтойка: x.currentRack })),
  });

  res.json({
    mag,
    racksCreated: created.length,
...
app.get('/api/health', async (req, res) => {
  ...
  res.json({ status: 'ok', service: 'switch-inventory-backend', oidcEnabled });
});
...
async function writeAudit(req, action, target, details) {
  try {
    await db.run(
      ...
      Date.now(),
      req.user ? req.user.username : 'система',
      action,
      target || null,
