...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    /** Сообщение о неудачном входе через поставщика. */
...
export const { sessionEstablished, logout, clearError, authFailed } = authSlice.actions;
/**
 * Завершение входа через корпоративного поставщика.
 *
 * Поставщик возвращает браузер с токеном в адресе. Токен сразу
 * сохраняется, а адрес очищается: иначе он остался бы в истории
 * браузера и мог попасть в закладки.
 */
export function consumeAuthRedirect() {
  return (dispatch) => {
    ...
    try {
      ...
      /* в изолированных средах изменение адреса может быть недоступно */
    }
    ...
    const messages = {
      error: 'Не удалось войти через корпоративную учётную запись.',
      expired: 'Время на вход истекло. Попробуйте ещё раз.',
    };
    dispatch(authFailed(messages[error] ?? messages.error));
  };
}
