...
-- ── Журнал действий ───────────────────────────────────────────────────
-- Фиксирует операции, меняющие данные — свои и чужие. Нужен, чтобы при
-- разбирательстве можно было установить, кто и что сделал: без него
-- ответа на такой вопрос не существует.
--
-- Особенно важен для отправки данных во внешние системы: там изменения
-- затрагивают чужую зону ответственности.
CREATE TABLE IF NOT EXISTS audit_log (
  id           SERIAL PRIMARY KEY,
  ts           BIGINT NOT NULL,
  username     TEXT NOT NULL,
  action       TEXT NOT NULL,   -- что сделано
  target       TEXT,            -- над чем
  details      TEXT,            -- подробности в свободной форме
  ip           TEXT
);

COMMENT ON COLUMN audit_log.action IS 'netbox_export | user_change | bulk_import | ...';
